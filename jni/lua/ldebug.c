...
static int precheck(const Proto *pt) {
    ...
    check(
            pt->sizecode > 0 && GET_OPCODE(pt->code[pt->sizecode-1]) == OP_RETURN);
    return 1;
}
    ...
    for (pc = 0; pc < lastpc; pc++) {
        ...
        case OP_LOADBOOL: {
            if (c == 1) { /* does it jump? */
                ...
                /* check its jump */
                check(
                        GET_OPCODE(pt->code[pc+1]) != OP_SETLIST || GETARG_C(pt->code[pc+1]) != 0);
            }
            break;
        }
        case OP_LOADNIL: {
        ...
        case OP_VARARG: {
            check(
                    (pt->is_vararg & VARARG_ISVARARG) && !(pt->is_vararg & VARARG_NEEDSARG));
            b--;
            if (b == LUA_MULTRET)
                check(checkopenop(pt, pc));
            checkreg(pt, a+b-1);
...
void luaG_typeerror(lua_State *L, const TValue *o, const char *op) {
    ...
    const char *kind =
            (isinstack(L->ci, o)) ?
                    getobjname(L, L->ci, cast_int(o - L->base), &name) : NULL;
    if (kind)
        luaG_runerror(L, "attempt to %s %s " LUA_QS " (a %s value)", op, kind,
                name, t);
    else
